#!/usr/bin/env python3
"""
Test Claude connection specifically.
"""
...
async def test_claude_basic():
    ...
    print("Testing basic Claude connection...")
    print(f"API Key: {settings.claude_api_key[:20]}...")
    ...
    try:
        ...
        print("SUCCESS: Claude connection working!")
        print(f"Response: {response.content}")
        return True

    except Exception as e:
        print(f"ERROR: Claude connection failed: {e}")
        return False

async def test_claude_sql_generation():
    """Test SQL generation with Claude."""
...
async def test_claude_model_alternatives():
...
async def main():
    ...
    basic_test = await test_claude_basic()

    if not basic_test:
        print("\nTrying alternative models...")
        working_model = await test_claude_model_alternatives()
        ...
        if working_model:
            ...
            print("2. Your Claude account has credits")
            print("3. Your internet connection")
            return

    # Test SQL generation if basic test passed
    if basic_test:
        await test_claude_sql_generation()

    print("\n" + "=" * 50)
    print("Claude tests completed!")
